    ...
    class Program {
        static void Main(string[] args) {
            ...
            using(new ClacksServer(new IPEndPoint(IPAddress.Parse("127.0.0.1"), port), statsCollector, clientHandlerFactory)) {
                ...
                var pool = ConnectionPool.Create("127.0.0.1", port, ConnectionTestingBehavior.Reconnect);
                var poolUseCount = 0;
                for(var i = 0; i < n; i++) {
                    ...
                    workers.Add(Task.Factory.StartNew(() => {
                        ...
                        while(true) {
                            ...
                            if(globalUseCount > r.Next(200, 300)) {
                                ...
                                pool = ConnectionPool.Create("127.0.0.1", port, ConnectionTestingBehavior.Reconnect);
                                myPool = pool;
